...
mouse_tree = read.tree("data/reference_sets/scripts/mus_musculus_129S1_v_genes.tree")
...
p1 = phyloseq::plot_tree(gpt_132_rooted, ladderize = TRUE, nodelabf = nodeplotblank) +
    ggtitle("gpt genes") +
    tree_theme + 
    scale_x_continuous(breaks = seq(0, .9, by = .2), limits = c(0, .9))
p2 = phyloseq::plot_tree(mouse_rooted, ladderize = TRUE, nodelabf = nodeplotblank) +
    ggtitle("Mouse strain 129S1 V genes") +
    tree_theme +
    scale_x_continuous(breaks = seq(0, .9, by = .2), limits = c(0, .9))
p3 = phyloseq::plot_tree(human_rooted, ladderize = TRUE, nodelabf = nodeplotblank) +
